# MiniDualSteamEngine
Assemblies – CAD Project
📌 Project Overview

This repository contains a complete 3D CAD assembly of a hydraulic cylinder piping system, designed and assembled using Siemens NX.
The project focuses on realistic industrial modeling practices, proper component hierarchy, and accurate assembly constraints suitable for manufacturing-oriented applications.

The assembly demonstrates fluid entry and exit piping integration with a hydraulic cylinder, commonly used in industrial machinery, automation systems, and heavy equipment.

🛠️ Tools & Software Used

CAD Software: Siemens NX
Modeling Approach: Parametric solid modeling
Assembly Type: Top-down & bottom-up hybrid
File Format: .prt (Siemens NX Part Files)

🧩 File Description

ASSEMBLY MAIN.prt
Master assembly file integrating all sub-assemblies and components.

final assembly.prt / assemblyfinal assembly.prt
Consolidated assemblies showing the complete system alignment and fit.

assembly cylinder pipe assembly tube entry.prt
Entry-side piping assembly connected to the hydraulic cylinder inlet.

assemblycylinder piping tube exit assembly.prt
Exit-side piping assembly for fluid discharge from the cylinder.

⚙️ Key Features

Fully constrained and aligned hydraulic cylinder assembly
Realistic piping layout for fluid entry and exit
Industry-oriented component structuring
Proper use of mates, alignments, and constraints
Clean and modular assembly design for easy modification
Suitable for manufacturing, simulation, and design reviews

🎯 Applications

Hydraulic systems design
Industrial machinery modeling
Manufacturing & assembly planning
CAD portfolio and academic projects
Learning reference for Siemens NX assemblies

🚀 How to Use

Open Siemens NX
Load ASSEMBLY MAIN.prt as the primary file
Ensure all part files are in the same directory
Use Assembly Navigator to explore sub-assemblies
Modify parameters or constraints as needed

📚 Learning Outcomes

Advanced assembly management in Siemens NX
Practical understanding of hydraulic system layout
Experience with real-world mechanical component integration
Improved CAD structuring and design intent clarity

👤 Author

Ayush Prajapati
Mechanical Engineering | CAD & Design Enthusiast
Skills: CAD Modeling, Siemens NX, Mechanical Design, Manufacturing Concepts

